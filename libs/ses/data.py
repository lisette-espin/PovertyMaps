...
class Data(object):
  ...
  def iterate_train_val(self, tune_img=True):
    ...
    for fn in files:
      ...
      for fold in foldcol:
        ...
        if (tune_img and SESImages.needs_tuning_runid_fold(path, self.model_name, epoch, rs, fold)) or not tune_img:
          ...
          yield train, val, path, epoch, rs, fold
  ...
  def load_metadata(self, viirsnorm=False):
    ...
    for fn in sorted(glob.glob(os.path.join(path,'clusters',f'*{self.prefix}*.csv'))):
      ...
      if fn.endswith("_cluster.csv"):
        cols = [c for c in tmp.columns if c not in [GTID,'year']]
    ...
    for fn in sorted(glob.glob(os.path.join(path,'pplaces','PPLACES*.csv'))):
      ...
      if fn.endswith("PPLACES.csv"):
        cols = [c for c in tmp.columns if c!=OSMID]
  ...
  def metadata_get_X(self, df=None):
    ...
    #df_data.rename(columns={'rural':'dhs_rural'}, inplace=True) # this is not necesary since rural is set by default in pplaces
  ...
  def metadata_get_X_y(self, df, y_attribute, fmaps, offlineaug=False, features_source='all', timevar=False):
    df_data = df.copy()
    ...
    # cluster features
    df_data = df_data.join(self.df_clusters, how='inner')
...
#     sys.exit(0)
    ...
    # rural feature
    df_data.drop(columns=['pplace_rural','cluster_year','cluster_number','pplace_cluster_distance','ses'], inplace=True)
    ...
    if offlineaug:
      ...
      for i,row in df_data.iterrows():
        ...
        if tmp.query(f"cluster_id == '{row.cluster_id}'").iloc[0].augmented:
