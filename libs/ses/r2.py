...
# adaptation from: https://github.com/tensorflow/addons/blob/v0.14.0/tensorflow_addons/metrics/r_square.py#L45-L198
...
class R2(Metric):
  ...
  def __init__(
      self,
      name: str = "r2",
      dtype: AcceptableDTypes = tf.float32,
      y_shape: list = [1,],
    ...
    self.squared_sum = self.add_weight(
      name="squared_sum", shape=self.y_shape, initializer="zeros", dtype=dtype
    )
    self.sum = self.add_weight(
      name="sum", shape=self.y_shape, initializer="zeros", dtype=dtype
    )
    self.res = self.add_weight(
      name="residual", shape=self.y_shape, initializer="zeros", dtype=dtype
    )
    self.count = self.add_weight(
      name="count", shape=self.y_shape, initializer="zeros", dtype=dtype
    )
