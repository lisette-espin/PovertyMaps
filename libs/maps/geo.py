...
def fast_find_nearest_per_record(gdfa, gdfb, dropgeometry=True, n_jobs=1):
    ...
    # nearest
    dist, idx = treeb.query(pointsa, k=1, workers=n_jobs)
    if dropgeometry:
...
def distribute_data_in_grid(gdf_data, gdf_grid, column, aggfnc='sum', lsuffix='pop', how='right'):
    
    # 1. spatial-join the data with the grid
    griddata = gpd.sjoin(gdf_data, gdf_grid, how=how, predicate='within', lsuffix=lsuffix, rsuffix='grid')

    if how=='right':
        griddata = griddata.reset_index().rename(columns={'index':'index_grid'})

    # 2. aggregate multiple data points into one (per grid-cell)
    griddata = griddata.groupby('index_grid')[column].agg(aggfnc)

    if type(aggfnc)==list:
        functions = ['mean','std','sum','min','max','nunique','mode','count']
        griddata = griddata.reset_index()
        griddata.rename(columns={c:f"{column}_{c}" for c in griddata.columns if c in functions}, inplace=True)
    else:
        griddata = griddata.reset_index(name=column)

    griddata = griddata.set_index('index_grid').join(gdf_grid)
    griddata = gpd.GeoDataFrame(griddata, geometry='geometry')
