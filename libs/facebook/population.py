...
class FacebookPopulation(object):
    ...
    def update_population_features(self, meters=METERS, n_jobs=1):
        '''
        ...
        #   gdf_nearest, dist = geo.fast_find_nearest_per_record(self.gdf_places_proj, self.gdf_pop_proj, n_jobs=n_jobs)
        #   pbar.update(1)  
        # print(gdf_nearest.head(10))
        # print("gdf_nearest:", gdf_nearest.shape, "| dist:", dist.shape)
        gdf = self.gdf_places_proj.sjoin_nearest(self.gdf_pop_proj[[self.population,'geometry']], how="left", distance_col="distance_closest_tile").copy()
        gdf.rename(columns={self.population:'population_closest_tile'}, inplace=True)
        ...
        for m in tqdm(meters, total=len(meters)):
            km = round(m/1000.,2)
            within = [self.gdf_pop_proj.iloc[list(self.index_pop.intersection(poly.bounds))].loc[:,self.population].sum() for poly in gdf.geometry.buffer(m/2., cap_style=3, join_style=1)]
            gdf.loc[:,f'population_in_{km}km'] = within

        gdf.loc[:,f'population_grav_1'] = gdf.apply(lambda row: row.population_closest_tile / row.distance_closest_tile, axis=1)
        gdf.loc[:,f'population_grav_1.5'] = gdf.apply(lambda row: row.population_closest_tile / row.distance_closest_tile**1.5, axis=1)
        gdf.loc[:,f'population_grav_2'] = gdf.apply(lambda row: row.population_closest_tile / row.distance_closest_tile**2, axis=1)
