...
def plot_counts(df, x, row, xindex=False, xlabel=None, ylabel=None, fn=None):
  #tmp = df.groupby(['urban','comm']).size().reset_index()
  #ax = tmp.plot.bar(legend=False, subplots=True)
  #ax.set_xticklabels([xl.get_text() for xl in ax.get_xticklabels()], rotation=xtickrot)
...
def plot_pred_true(pred, true, metrics, fn=None):
  ...
  with sns.plotting_context("paper", font_scale=FONT_SCALE):
    ...
    for c in np.arange(cols):
      ...
      mse = mean_squared_error(ctrue,cpred)
...
def plot_learning_curves(history,fn=None):
  ...
  for i, k in enumerate(metrics):
    ...
    ax.plot(history.history[f'val_{k}'], color='orange', label='val')
    ax.set_title(k)
  plt.legend()
