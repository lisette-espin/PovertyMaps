...
def get_data_and_places_file(root, years, source):
  ...
  if source == 'connectivity':
    fn_data = glob.glob(os.path.join(root,source,"cell_towers_{}.csv".format(root.split("/")[-1])))
  elif source == 'population':
    fn_data = glob.glob(os.path.join(root,source,"Facebook","population_*.csv.zip"))
...
def get_places_file(root, years=None):
  ...
  if years in ['',None,np.nan]:
    ...
    for fn in files:
      ...
      years_in_fn = sum([1 for year in years if year in fn]) # from 'years' how many are in fn
...
def save_json(obj, fn, verbose=True):
    try:
        with open(fn,'w') as f:
            json.dump(obj, f)
        if verbose:
