...
from utils.constants import SIMILARITY_DISTANCES
from utils.validations import validate_y_attribute
...
N_QUANTILES = 5
FSOURCE_IMAGE = 'GMSA'
FEATURE_MAP = OrderedDict({'FBM':'De', 'FBMV':'Mo', 'FBP':'Po', 'NTLL':'Nl', 'OCI':'An', 'OSM':'In', FSOURCE_IMAGE:'Im'})
...
CB_CLASS = [v for k,v in MODELS.items() if k in ['catboost','weighted_catboost']]
ALL_MODELS = [v for k,v in MODELS.items()] #['CB', 'CB$_w$', 'CNN', 'CNN$_a$', 'CNN+CB', 'CNN$_a$+CB', 'CNN+CB$_w$', 'CNN$_a$+CB$_w$']
...
COMBINED_FEATURES = ['images+metadata']
METADATA_COMBINED_FEATURES = METADATA_FEATURES + COMBINED_FEATURES
...
RECENCY_BEST = None #RECENCY['combined']
...
WB_INDICATORS = {'Sierra Leone':{'gini':'35.7 (2018)', 'gdp_per_capita':'515 (2016), 520 (2018), 507 (2019)', 'gdp_growth':'6.3 (2016), 3.5 (2018), 5.3 (2019)'}, #2016, 2018, 2019
                 'Uganda':      {'gini':'42.7 (2019)', 'gdp_per_capita':'754 (2016), 793 (2018), 823 (2019)', 'gdp_growth':'4.8 (2016), 6.3 (2018), 6.4 (2019)'}, # 2016, 2018, 2019
                 
                 'Liberia':      {'gini':'35.3 (2016)', 'gdp_per_capita':'666 (2019), 755 (2022)', 'gdp_growth':'-2.5 (2019), 4.8 (2022)'}, #2016, 2019, 2022
                 'Rwanda':      {'gini':'43.7 (2016)', 'gdp_per_capita':'806 (2019)', 'gdp_growth':'9.5 (2019)'}, #2016, 2019
                 'Gabon':      {'gini':'38.0 (2017)', 'gdp_per_capita':'7524 (2019)', 'gdp_growth':'3.9 (2019)'}, #2016, 2019
                 'South Africa': {'gini':'63.0 (2014)', 'gdp_per_capita':'2734.5 (2017)', 'gdp_growth':'1.2 (2017)'},  #2014, 2017, ...
                }

AFRICAN_COUNTRIES_ORDER = ['Sierra Leone','Liberia','Gabon','South Africa','Rwanda','Uganda']
...
def get_performance_from_predictions(df_rs, output=None):
  ...
  if fn is not None and ios.exists(fn):
    ...
    for group, df in df_rs.groupby(['country','model','relocation','recency','weighted',
                                    'augmented','features','features_source','epoch']):
        n = df.shape[0]

        if n > 0:
            tmp = df.drop(columns=[c for c in df.columns if c in ['rural','year','cluster_id'] or 
                                   c.startswith('pred') or c.startswith('true') or c.startswith('residual') or 
                                   c.startswith('population')]).copy()
    ...
    cols = ['gt_config', 'relocation', 'recency', 'weighted', 'augmented',
            'features', 'features_source', 'epoch', 'rs', 'model', 'fn', 
            'r2', 'r2_mean_wi','r2_std_wi', 
            'rmse', 'rmse_mean_wi', 'rmse_std_wi', 
            'nrmse', 'nrmse_mean_wi', 'nrmse_std_wi', 
            'mse','mse_mean_wi', 'mse_std_wi', 'country', 'country_code']
...
def get_performance(root=None, countries=None, fsource='all', output=None):
  ...
  if fn is not None and ios.exists(fn):
    ...
    for r,country in enumerate(countries):
      tmp = summary_from_logs(root, country, fsource)
...
def get_summary_performance(df, only_best=True, metric='mse', features_source='all', output=None):
  ...
  if fn is not None and ios.exists(fn):
    ...
    if RECENCY_BEST is not None:
        ...
        q = " or ".join([f"(country_code=='{ccode}' and recency=='{best_recency}')" for ccode,best_recency in BEST_RECENCY.items()])
        df_rel = df_rel.query(q) if only_best else df_rel
    df_rel = df_rel.groupby(group_rel).agg({metric:['mean','std'], 
                                            f"{metric}_mean_wi":['mean','std'], 
                                            f"{metric}_std_wi":['mean','std'],}).reset_index()
    ...
    # augmented
    group_aug = ['country','model','recency','relocation','weighted','augmented']
    df_aug = data.query("model in @CNN_CLASS and weighted=='no' and features in @IMAGE_FEATURE and relocation==@RELOCATION_BEST").copy()
    if RECENCY_BEST is not None:
        df_aug = df_aug.query("recency==@RECENCY_BEST") if only_best else df_aug
    else:
        q = " or ".join([f"(country_code=='{ccode}' and recency=='{best_recency}')" for ccode,best_recency in BEST_RECENCY.items()])
    df_aug = df_aug.query(q)    
    #df_aug = df_aug.query("recency==@RECENCY_BEST and relocation==@RELOCATION_BEST") if only_best else df_aug
    df_aug = df_aug.groupby(group_aug).agg({metric:['mean','std'], 
                                            f"{metric}_mean_wi":['mean','std'], 
                                            f"{metric}_std_wi":['mean','std'],}).reset_index()
    ...
    # weighted samples
    group_wei = ['country','model','recency','relocation','augmented','weighted']
    df_wei = data.query("model in @MODELS_CB and features in @METADATA_FEATURES and features_source == @features_source and relocation==@RELOCATION_BEST").copy()
    if RECENCY_BEST is not None:
        df_wei = df_wei.query("recency==@RECENCY_BEST") if only_best else df_aug
    else:
        q = " or ".join([f"(country_code=='{ccode}' and recency=='{best_recency}')" for ccode,best_recency in BEST_RECENCY.items()])
    df_wei = df_wei.query(q)  
    #df_wei = df_wei.query("recency==@RECENCY_BEST and relocation==@RELOCATION_BEST") if only_best else df_wei
    df_wei = df_wei.groupby(group_wei).agg({metric:['mean','std'], 
                                            f"{metric}_mean_wi":['mean','std'], 
                                            f"{metric}_std_wi":['mean','std'],}).reset_index()
...
def get_residuals(root, countries, country_metadata=None, fsource='all', output=None):
  ...
  if fn is not None and ios.exists(fn):
    ...
    df = pd.DataFrame()
    for country in countries:
      tmp = summary_from_predictions(root, country, fsource)
      tmp.loc[:,'country'] = country
      
      tmp.loc[:,'country_code'] = None
      if country_metadata is not None:
        tmp.loc[:,'country_code'] = tmp.country.apply(lambda v: country_metadata[v]['code'])
  ...
  df.loc[:,'features_source'] = df.loc[:,'fn'].apply(lambda v: v.split('xgb-')[-1].split('/')[0] if 'xgb' in v else 
                                                     FSOURCE_IMAGE if 'cnn' in v and 'xgb' not in v else '-')
...
def get_recency_code(gt_config):
  ### @TODO: check with resources/survey/available_data.json
...
def get_features_type(fn, fsource='all'):
  iscnn = 'cnn' in fn
  individualsource = fn.split('xgb-')[-1].split('/')[0] 
  isindividualsource = individualsource in FEATURE_MAP.keys()
  ismetadata = 'xgb' in fn and not isindividualsource #in [f'xgb-{fsource}', 'xgb-all']
  
  ft = 'images' if iscnn and not ismetadata else \
         'metadata' if not iscnn and ismetadata else \
         'images+metadata' if iscnn and ismetadata else \
         individualsource if isindividualsource else None
   
  return ft


def summary_from_logs(root, country, fsource='all'):
  ...
  for fn in files:
    try:
      ...
      features = get_features_type(fn, fsource)
...
def summary_from_predictions(root, country, fsource='all'):
  ...
  for fn in files:
    try:
      ...
      year = data.cluster_year.apply(lambda v: str(v) if type(v)==int else '-'.join(v).replace('-20','-') if type(v)==list else v)
      #apply(lambda v: '2016' if str(v)=='2016' else '2018-19')
...
def plot_poverty_map(ccode, model, years, features_source='all', var='mean', label=None, output=None, ext='pdf'):
  
  validate_y_attribute(var)
  model = model.replace('$_w$','w').replace('$_a$','a')

  # prediction files on pplaces
  fs = '' if model in CNN_CLASS else f"_{features_source}"
  fn = f'results/pplaces_inference/{ccode}_{model}_{years}{fs}.csv'
  df1 = ios.load_csv(fn)
    
  # pplaces' features
  fn = f'results/pplaces_inference/{ccode}_features_{features_source}.csv'
  df2 = ios.load_csv(fn, index_col=None)
  ...
  n = df.shape[0]
  vmin, vcenter, vmax, vstd = df[f'pred_{var}_wi'].min(), df[f'pred_{var}_wi'].mean(), df[f'pred_{var}_wi'].max(), df[f'pred_{var}_wi'].std()
  norm = TwoSlopeNorm(vmin=vmin, vcenter=vcenter, vmax=vmax)
  cmap = 'BrBG'
  #print(ccode, vmin, vcenter, vmax)
  
  # plot
  pad = {'UG':-0.2, 'SL':-0.1, 'LB':-0.1, 'RW':-0.1, 'ZA':-0.1}
  anchorx = {'UG':0.8, 'SL':0.15, 'LB':0.15, 'RW':0.90, 'ZA':0.15}
  gdf.plot(column=f'pred_{var}_wi', 
           ax=ax,
           # cax=cax,
           markersize=5, 
           vmin=0, vmax=100,
           ...
           legend_kwds={'shrink':0.25, 'location':'bottom', 
                        ...
                        #'label':'IWI mean',
                        'pad':pad[ccode],
                        'anchor':(anchorx[ccode], 1.0)}
          )
  fig.axes[1].tick_params(labelsize=11)
  ax.set_axis_off()
    
  label = f"({label})" if label is not None else ''
  title = f"{label} {[c for c,obj in COUNTRIES.items() if obj['code']==ccode][0]}"
  ...
  if output is not None:
    fn = os.path.join(output,f"sm_hr_poverty_map_{ccode}_{model}_{years}{fs}_{var}.{ext}")
    fig.savefig(fn, dpi=300, bbox_inches='tight')
    print(f"{fn} saved!")
...
def plot_summary_performance(df, metric='nrmse', countries_order=None, output=None):
  ...
  fg = sns.catplot(data=data.sort_values('value', ascending=True), 
                   kind='bar',
                   col="kind", col_order=['recency','relocation','augmented','weighted'],
                   row="country", row_order=countries_order,
...
def plot_variability(df_rs, df_gt, df_dr, metric='nrmse', features_source='all', countries_order=None, output=None):
  ### Annotate start
  ...
  countries_order = [c for c in COUNTRIES.keys() if c in df_gt.country.unique()]

  # Only necesary data (best models) - test set (top gt, bottom prediction)
  q1 = "relocation==@RELOCATION_BEST and features in @MAIN_FEATURES"
  
  # Selecting the best models
  best_models = select_best_model(df_dr, features_source, metric, countries_order)
  # print(best_models)
  
  if RECENCY_BEST is not None:
    q2 = f"recency=='{RECENCY_BEST}'"     
    _q = []
    for country, row in best_models.iterrows():
      _s = f"(model=='{row.model}' and country=='{country}' and features_source=='{row.features_source}')"
      _q.append(_s)
    q3 = " or ".join(_q)
    ...
    _q = []
    for country, row in best_models.iterrows():
      _s = f"(model=='{row.model}' and country=='{country}' and features_source=='{row.features_source}' and recency=='{BEST_RECENCY[COUNTRIES[country]['code']]}')"
      _q.append(_s)
    q2 = " or ".join(_q)
  ...
  fg = sns.FacetGrid(data=data, col='country', col_order=countries_order,
                     row='kind', hue='settlement', hue_order=['urban','rural'], 
  ...
  ylabel = 'IWI_std'
  qold = 0
  for c, ax in enumerate(fg.axes[0,:]):
    country = ax.get_title()
    print(country)
    ...
    for i,q in enumerate(quantiles):
      ...
      for r, kind in enumerate(data.kind.unique()):
        ...
        if i in [1,5]:
          ax.text(s=f'Q{i}', x=(q+qold)/2, y=1.0, transform=ax.get_xaxis_transform(), ha='center', c='lightgrey',va='top',size=12, bbox=dict(facecolor='white', edgecolor='none', pad=2.5)) 
      qold = q.copy()
...
def get_summary_all_performances(df_performance_within, df_performance_cross, metric='rmse', fsource='all', fn=None):
    
    ### WITHIN
    fs = fsource.split('_')
    
    q1 = " relocation=='none'"
    q2 = " or ".join([f"(country_code=='{c}' and recency=='{r}')" for c,r in BEST_RECENCY.items()]) 
    q3 = " (features_source == @fsource and model in @CB_CLASS and features=='metadata') or (features in @fs and model in @CB_CLASS) or (features in ['images','images+metadata']) "
    q_within  = f"({q1}) and ({q2}) and ({q3})"

    data_within_summary = df_performance_within.query(q_within).groupby(['country', 'features', 'model'])[metric, f'{metric}_mean_wi', f'{metric}_std_wi'].mean()
    data_within_summary = data_within_summary.pivot_table(index=['model', 'features'], columns=['country']) #
    if fn is not None:
        data_within_summary.reset_index().to_csv(fn, index=0)

    ### WITHIN VALID
    q4 = " ((features_source == @fsource and model in @CB_CLASS and features=='metadata') or (features in ['images','images+metadata'])) "
    q_valid_within  = f"({q1}) and ({q2}) and ({q4})"
    data_valid_within = df_performance_within.query(q_valid_within).copy()
    data_valid_within = data_valid_within.groupby(['country','relocation','recency','weighted','augmented','features','features_source','model']).mean().dropna().reset_index()
    
    ### SUMMARY
    cols = ['source_country','target_country','model','within_source','within_target','across']
    df_performances_summary = pd.DataFrame(columns=cols)
    for group, tmp in df_performance_cross.groupby(['source_model','source_country','target_country']):
        model, source_country, target_country = group

        recency_source = BEST_RECENCY[COUNTRIES[source_country]['code']]
        performance_within_source = data_valid_within.query("country==@source_country and recency==@recency_source and relocation=='none' and model==@model")[metric].iloc[0]

        recency_target = BEST_RECENCY[COUNTRIES[target_country]['code']]
        performance_within_target = data_valid_within.query("country==@target_country and recency==@recency_target and relocation=='none' and model==@model")[metric].iloc[0]

        performance_across = tmp[metric].mean()

        obj = {'source_country':COUNTRIES[source_country]['code'],
               'target_country':COUNTRIES[target_country]['code'],
               'model':model,
               'within_source':performance_within_source,
               'within_target':performance_within_target,
               'across':performance_across}

        df_performances_summary = pd.concat([df_performances_summary[cols], pd.DataFrame(obj, index=[0], columns=cols)], ignore_index=True)
    return data_valid_within, data_within_summary, df_performances_summary
...
def plot_model_transfer_performances(df_summary, countries, metric, best_distance_country=None, models=ALL_MODELS):
    
    ncountries = len(countries) #df_summary["target_country"].nunique()
    nmodels = len(models)

    w = 2.5
    h = 2.5 if nmodels > 1 else 3.0
    fig, axes = plt.subplots(nmodels, ncountries, figsize=(w*ncountries, h*nmodels), sharex=True, sharey=True)

    for row, model in enumerate(models):
        for column, target in enumerate(countries):
            tmp_performance = df_summary.query("target_country==@target and model==@model")
            ax = axes[row, column] if nmodels > 1 else axes[column]

            if tmp_performance[f"within_target"].nunique() > 1:
                raise Exception("Something went wrong")

            ax.axhline(tmp_performance[f"within_target"].unique()[0], ls='--', lw=1, c='grey')

            if best_distance_country:
                distance_key = best_distance_country[target]['best_distance']
                distance_matrix = best_distance_country[target]['distance_matrix']
                tmpdm = distance_matrix.loc[target].drop(target)
                best_distance = tmpdm.min() if distance_key not in SIMILARITY_DISTANCES else tmpdm.max()
                best_across = tmpdm.idxmin() if distance_key not in SIMILARITY_DISTANCES else tmpdm.idxmax()
                best_country_id = countries.index(best_across)
                
                
                # tmpdm = distance_matrix.loc[target].drop(target)
                # best_country = tmpdm.idxmin() if distance_key not in SIMILARITY_DISTANCES else tmpdm.idxmax()
                # best_country_id = countries.index(best_country)
                
                valy = tmp_performance.query(f"source_country==@best_across").iloc[0].across
                ax.text(s=distance_key, x=best_country_id, y=valy+0.1, va='bottom', ha='center', c='darkorange') #, transform=ax.get_xaxis_transform()) #transform=ax.transAxes)

            x = []
            y = []
            c = []
            across = sorted(tmp_performance.across)
            first = across[0]
            second = across[1]
            third = across[2]
            for i, source in enumerate(countries):
                valy = tmp_performance.query(f"source_country==@source").iloc[0].across if source!=target else tmp_performance.iloc[0].within_target
                
                x.append(i)
                y.append(valy)
                
                if best_distance_country:
                    c.append('darkorange' if source!=target and source==best_across else 
                             'black' if source==target else 'navajowhite')
                else:
                    cacross = tmp_performance.query(f"source_country==@source").iloc[0].across if source!=target else None
                    c.append('darkorange' if source!=target and cacross==first else 
                             'lightgreen' if source!=target and cacross==second else 
                             'blue' if source!=target and cacross==third else 
                             'black' if source==target else 'lightgrey')
            ax.scatter(x=x, y=y, c=c) 

            if column == 0:
                ax.set_ylabel(metric.upper())
            if row == nmodels-1 and column == int(ncountries/2):
                ax.set_xlabel('Source country')
            if column == ncountries-1:
                ax2 = ax.twinx()
    ...
    plt.suptitle(f"Model transfer", y=0.99)
    fig.tight_layout()
...
# Selecting the best models
def select_best_model(df_dr, features_source, metric, countries_order=COUNTRIES.keys()):
    col_metrics = ['nrmse','nrmse_mean_wi','nrmse_std_wi','rmse','rmse_mean_wi','rmse_std_wi','mse','mse_mean_wi','mse_std_wi','r2','r2_mean_wi','r2_std_wi']
    
    q1 = "(relocation==@RELOCATION_BEST)"
    q2 = " and ((features_source==@features_source and features!='images') or (features=='images'))"
    
    if RECENCY_BEST is not None:
        q3 = f" and (recency==@RECENCY_BEST)"
    else:
        q3 = " or ".join([f"(country_code=='{ccode}'  and  recency=='{best_recency}')" for ccode,best_recency in BEST_RECENCY.items()])
        q3 = f" and ({q3})"
    
    q = f" {q1} {q2} {q3} "
    tmp = df_dr.query(q).groupby(['country','weighted','augmented','model','features','features_source'])[col_metrics].mean().reset_index().dropna()
    
    if metric == 'r2':
        # larger values the better
        best_models = tmp.loc[tmp.groupby('country')[f'{metric}_mean_wi'].idxmax()].set_index('country')
    else:
        # lower values (errors) the better
        best_models = tmp.loc[tmp.groupby('country')[f'{metric}_mean_wi'].idxmin()].set_index('country')
        
    return best_models.loc[[c for c in countries_order if c in best_models.index]]
...
def tbl_data_summary(root, countries, years, output=None):
  ...
  for country in countries:
    ...
    # population
    fn_population_clusters = [fn for fn in glob.glob(os.path.join(root,'../','resources', 'Facebook', 'Population', country, "*_general_2020_csv.zip"))][0]
    df_population = ios.load_csv(fn_population_clusters, index_col=None)
    cpop = [c for c in df_population.columns if c not in ['longitude','latitude']][0]
    popfb = df_population[cpop].sum().round(0)
    print(f"[INFO] {fn_population_clusters} loaded!")
    ...
    obj = {'country':country,
           ...
           'population (FB)':popfb,
           'population / populated places':round(popfb/pp,0),
           
           'pplaces urban':f"{ppu} ({ppu*100/pp:.0f}%)",
           'pplaces rural':f"{ppr} ({ppr*100/pp:.0f}%)",
           'relocation':f"{rl} ({rl*100/c:.0f}%)" if rl else None,
           'relocation urban':f"{rlu} ({rlu*100/cu:.0f}%)" if rlu else None,
           'relocation rural':f"{rlr} ({rlr*100/cr:.0f}%)" if rlr else None,
          }
    df_summary = pd.concat([df_summary,pd.DataFrame(obj,index=[1])], ignore_index=True)
...
def tbl_performance_by_model(df, metric='rmse', features_source='all', include_baselines=False, output=None):
  
  metric = validate_metric(metric)
  
  q1 = f"(relocation==@RELOCATION_BEST) and (features in @MAIN_FEATURES) and ((features in @METADATA_COMBINED_FEATURES and features_source==@features_source) or (features in @IMAGE_FEATURE))"
  q2 = "" if include_baselines else "and (weighted=='yes' or augmented=='yes')"
  ...
  q = f"{q1} {q2} {q3}"
  data = df.query(q, engine='python').copy()
...
def tbl_performance_by_datasource(df, metric='rmse', features_source='all', include_images=False, output=None):
  
  metric = validate_metric(metric)
  q0 = "or model==@MODEL_CNNA" if include_images else ''
  q1 = f"(relocation==@RELOCATION_BEST) and (features not in @MAIN_FEATURES {q0})"
  
  if RECENCY_BEST is not None:
    q2 = f" and (recency==@RECENCY_BEST)"
  else:
    q2 = " or ".join([f"(country_code=='{ccode}'  and  recency=='{best_recency}')" for ccode,best_recency in BEST_RECENCY.items()])
    q2 = f" and ({q2})"
    
  q = f" {q1} {q2} "
  data = df.query(q).copy()
    
    
  if features_source!='all':
    valid_features = features_source.split('_') if "_" in features_source else features_source.split(",") if "," in features_source else None
    if valid_features is None:
        raise Exception("features_source is None")
  else:
    valid_features = list(FEATURE_MAP.keys())
  print("valid_features: ",valid_features)
  data = data.query("features in @valid_features")
  index = data.loc[:,['model','features']].drop_duplicates()
  data = data.groupby(['country','model','features']).mean().reset_index()
...
def tbl_performance_by_model_and_datasource(df, metric='rmse', features_source='all', overall=True, output=None):
  df_results_model = tbl_performance_by_model(df=df, metric=metric, features_source=features_source, include_baselines=True, output=output).reset_index()
  df_results_source = tbl_performance_by_datasource(df=df, metric=metric, features_source=features_source, include_images=False, output=output).reset_index()
  
  valid_features = []
  for k,v in FEATURE_MAP.items():
    
    if features_source=='all' or k in features_source.split("_") or k in features_source.split(","):
...
def assign_classes_per_country(df, df_gt, n_classes=3, class_type='class', var='mean',show_warning=False):
  class_type = validate_class_type(class_type)
  var = validate_var(var)
  labels = np.arange(1,n_classes+1,1)
  country = df.country.unique()[0] #', '.join(df.country.unique())
...
def tbl_intersectionality(df_rs, df_gt, country_name=None, features=None, features_source=None, 
                          n_classes=3, class_type='class', var='mean', evaluation='recall', output=None):
  evaluation = validate_evaluation(evaluation)
  models = MODELS.values()
  
  query = []
  q0 = f"relocation=='{RELOCATION_BEST}' and model in {list(models)}"
  q1 = f"country=='{country_name}'" if country_name else None
  q2 = " or ".join([f"(country_code=='{c}' and recency=='{r}')" for c,r in BEST_RECENCY.items()]) if RECENCY_BEST is None else f"recency=='{RECENCY_BEST}'"

  if features and features_source:
    q3 = f"features in {features} and ((features_source == @features_source and features in @METADATA_COMBINED_FEATURES) or (features_source == @FSOURCE_IMAGE))"
    q4 = None
  else:
    q3 = f"features in {features}" if features else None
    q4 = f"features_source == @features_source" if features_source else None
  ...
  data = df_rs.query(query, engine='python').copy()
  ...
  for country, df in data.groupby('country'): 
    ...
    df, labels, class_type = assign_classes_per_country(df, df_gt, n_classes, class_type, var, evaluation in ['recall','precision','f1'])
...
def tbl_cross_testing(df_dr, metric='r2', output=None, save=True):
  ...
  for (source_country, source_model), _ in df_cr.groupby(['source_country','source_model']):
    ...
    tmp.loc[:,'features_source'] = tmp.fn.apply(lambda v: v.split('/xgb-')[-1].split('/')[0] if 'xgb' in v else '-')
  ...
  for var in [None,'mean','std']:
    ...
    renamecols = {c:f"{COUNTRIES[c]['code']}_{column}"for c in tmp.source_country.unique()}
    tmp.rename(columns=renamecols, inplace=True)
    tmp.columns.name = ''

    new_cols = list(renamecols.values())
