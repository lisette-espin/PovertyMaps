...
def run(root, years, dhsloc, traintype, kfolds, epochs=None, random_states=None):
  ...
  df_mapping = df_mapping.query("dhs_year in @years").copy()
  print(f"{df_mapping.shape[0]} records in {years}.")
  prefix = "_".join([pre for pre in prefix.split('_') for y in years if y in pre])
  
  # internal stratified sampling
  bins = 10
  df_mapping.loc[:,'ses'] = pd.cut(df_mapping.dhs_mean_iwi, bins=bins, labels=np.arange(bins), precision=0, retbins=False, ordered=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-e", help="# Epochs (repeats)", type=int, default=None, required=False)
    parser.add_argument("-s", help="Random seed", default=None, required=False)
